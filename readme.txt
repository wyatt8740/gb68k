...
Once you have a ROM, it needs to be converted with the included utility.  It runs from the command line and has two parameters.  The first is the name of the ROM file; the second is the on calc name you would like.  This second name must be 6 characters or less.  The utility will generate a number of files with the name you specify, appended with numbers.  For example, if you run 'CartConvert mygame.gb mygame', you'll get files mygame00, mygame01, mygameXX.  For this to work, mygame.gb will need to be in the same directory as CartConvert.  The generated files will also be in this directory.  The exact number will depend on the size of the original ROM.  Send all these files to your calculator.  They should be archived, but they don't have to be.

Send the emulator to your calculator, make sure you have lots of free memory, and run it.  You'll get a list of all the ROMs on your calculator.  All folders will be scanned.  Select the game you want.  If all goes well, you should be playing.  If not, the emulator will quit with an error message, or maybe crash =(

Press [ESC] during emulation to open the menu.  Here you can change options, or quit the emulator.  The options are frame skip, timer enable, show fps, and archive sram.  Higher frame skips will make the game play faster, but less smooth.  All previous versions have used a frame skip of 2.  I recommend either 2 or 3.  Disabling the timer will cause some games to run faster (like marioland 1), but will cause some games not to run at all.  Most games won't care one way or the other.  You should always try to run a game without the timer, and then enable it only if needed.  Archive sram will cause all config files generated by the emulator to be automatically archived.  This is good unless you're worried about the life of your flash.  Show fps does what it sounds like.
...
* Fixed typo casuing mode1 to not be set in LCDC
...
* Moved gameboy SP into a 68k address register.  This gives a noticable speed increase on most games, but could potentially cause crashes or other issues.
...
v0.4.0 5/30/2005
* Added timer support.  The timing is not particulatly accurate, but it should be good enough for most games to work (such as SQRXZ and Montezuma's Return).  More importantly, the speed of games that don't use the timer isn't affected
