...
int main(int argc, void *argv[])
{
	...
	if(argc > 3 && strcmp((const char *)argv[3], "-pack") == 0) {
		banks_per_file = 7;
	} else {
		banks_per_file = 6;
	}
	...
	while(half_banks) {
		...
		if(half_banks & 1) more = true;
		else more = false;
		
		code_size = 1024 * 8 * current_banks + 2;
		patched_code = new unsigned char[code_size + 6000];
		*(unsigned char *)(patched_code + 0) = 0x00;
		...
		if(more && banks_per_file == 7) {
			...
			memcpy(patched_code + code_size, rom + offset, 6000);
			code_size += 6000;
			printf("yo");
